...
FEAST <- function(source = sources_data, sinks = sinks, em_itr = 1000, env = rownames(sources_data), include_epsilon = T, COVERAGE,
  ...
  if(include_epsilon == TRUE){
    ...
    sinks_rarefy = rarefy(as.data.frame(matrix(sinks, nrow = 1)), maxdepth = apply(totalsource_old, 1, sum)[1]) #make
    ...
    unknown_source_rarefy = rarefy(as.data.frame(matrix(unknown_source, nrow = 1)), maxdepth = COVERAGE)
